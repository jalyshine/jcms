...
<div class="layui-card">
	...
	<div class="layui-card-body">
		<form id="searchForm" action="${ base }content/Picture/list" method="get" class="layui-form">
		...
		<div class="layui-btn-group item-oper">
			<button class="layui-btn layui-btn-danger" data-type="delete">批量删除图片</button>
		</div>
		<form action="" method="post" id="operForm" class="layui-form">
			...
			<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
				...
				<tbody>
				<c:forEach items="${ page.list }" var="picture">
					<tr>
						<td style="text-align: center;">
							<input type="checkbox" name="ids" value="${ picture.id }" lay-skin="primary">
						</td>
						<td>${ picture.id }</td>
						<td>${ picture.title }</td>
						<td>${ picture.publishTimeStr }</td>
						<td>
							<a href="${ base }content/PictureComment/list?id=${ picture.id }"
							   class="layui-btn layui-btn-xs">评论</a>
							<a href="${ base }content/Picture/edit?id=${ picture.id }&cid=${ picture.categoryId }"
							   class="layui-btn layui-btn-xs">编辑</a>
							<a href="javascript:;" lay-data="${ picture.id }" name="delete"
...
<form action="${ base }content/Picture/delete" method="post" id="delForm">
	<input type="hidden" name="odr" value="${ param.odr }">
	<input type="hidden" name="tts" value="${ param.tts }">
	<input type="hidden" name="cid" value="${ category.id }">
...
<script>
    layui.use(['layer', 'form', 'laypage', 'laydate', 'element'], function () {
        ...
        $(".item-oper .layui-btn").on("click", function () {
            var action = "${ base }content/Picture/batch-" + $(this).data('type');
