...
<div class="layui-card">
	...
	<div class="layui-card-body">
		<form id="searchForm" action="${ base }content/Article/list" method="get" class="layui-form">
		...
		<div class="layui-btn-group item-oper">
			<button class="layui-btn layui-btn-danger" data-type="delete">批量删除文章</button>
		</div>
			...
			<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
				...
				<c:forEach items="${ page.list }" var="article">
					<tr>
						...
						<td>${ article.id }</td>
						<td>${ article.title }</td>
						<td>${ article.publishTimeStr }</td>
						<td>
							<a href="${ base }content/ArticleComment/list?id=${ article.id }"
							   class="layui-btn layui-btn-xs">评论</a>
							<a href="${ base }content/Article/edit?id=${ article.id }&cid=${ article.categoryId }"
							   class="layui-btn layui-btn-xs">编辑</a>
							<a href="javascript:;" lay-data="${ article.id }" name="delete"
...
<form action="${ base }content/Article/delete" method="post" id="delForm">
	<input type="hidden" name="odr" value="${ param.odr }">
	<input type="hidden" name="tts" value="${ param.tts }">
	<input type="hidden" name="cid" value="${ category.id }">
...
<script>
	layui.use(['layer', 'form', 'laypage', 'laydate', 'element'], function () {
		...
		$(".item-oper .layui-btn").on("click", function () {
            var action = "${ base }content/Article/batch-" + $(this).data('type');
