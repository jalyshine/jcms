...
<div class="layui-card">
	...
	<div class="layui-card-body">
		<form id="searchForm" action="${ base }content/Video/list" method="get" class="layui-form">
		...
		<div class="layui-btn-group item-oper">
			<button class="layui-btn layui-btn-danger" data-type="delete">批量删除视频</button>
		</div>
				...
				<c:forEach items="${ page.list }" var="video">
					<tr>
						<td style="text-align: center;">
							<input type="checkbox" name="ids" value="${ video.id }" lay-skin="primary">
						</td>
						<td>${ video.id }</td>
						<td>${ video.title }</td>
						<td>${ video.publishTimeStr }</td>
						<td>
							<a href="${ base }content/VideoComment/list?id=${ video.id }"
							   class="layui-btn layui-btn-xs">评论</a>
							<a href="${ base }content/Video/edit?id=${ video.id }&cid=${ video.categoryId }"
							   class="layui-btn layui-btn-xs">编辑</a>
							<a href="javascript:;" lay-data="${ video.id }" name="delete"
...
<form action="${ base }content/Video/delete" method="post" id="delForm">
	<input type="hidden" name="odr" value="${ param.odr }">
	<input type="hidden" name="tts" value="${ param.tts }">
	<input type="hidden" name="cid" value="${ category.id }">
...
<script>
    layui.use(['layer', 'form', 'laypage', 'laydate', 'element'], function () {
        ...
        $(".item-oper .layui-btn").on("click", function () {
            var action = "${ base }content/Video/batch-" + $(this).data('type');
