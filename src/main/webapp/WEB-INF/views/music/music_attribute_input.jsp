...
<body>
<c:if test="${ musicAttribute.id == null }">
	<c:set value="${ base }music/MusicAttribute/add" var="save"/>
</c:if>
<c:if test="${ musicAttribute.id != null }">
	<c:set value="${ base }music/MusicAttribute/edit" var="save"/>
</c:if>
<div class="layui-card">
	<div class="layui-card-header">
        <span class="card-title">添加 & 编辑类别</span>
		<c:import url="../menubar.jsp?module=music" />
	</div>
	<div class="layui-card-body">
		<form:form action="${ save }" method="post" modelAttribute="musicAttribute" cssClass="layui-form">

			<!-- 防止表单重复提交 -->
			<jfp:token/>
			<form:hidden path="id" value="${ musicAttribute.id }"/>
			<form:hidden path="attrName" value="${ musicAttribute.attrName }"/>
