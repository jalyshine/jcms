...
<div class="layui-card">
	<div class="layui-card-header">
        <span class="card-title">音乐专辑</span>
		<c:if test="${ param.sid != null }">
			<c:import url="music_singer_menubar.jsp?sid=${ param.sid }&entity=MusicAlbum&name=专辑" />
		</c:if>
		<c:if test="${ param.sid == null }">
		<span class="layui-breadcrumb">
			<a href="javascript:;"><cite>所有专辑</cite></a>
		</span>
		</c:if>
	</div>
	<div class="layui-card-body">
		<form id="searchForm" action="${ base }music/MusicAlbum/list" method="post" class="layui-form">
		...
		<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
			...
			<tbody>
			<c:forEach items="${ page.list }" var="album">
				<tr>
					<td>${ album.id }</td>
					<td>${ album.title }</td>
					<td>${ album.musicSinger.name }</td>
					<td>
						<fmt:formatDate value="${ album.updateTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td>${ album.hits }</td>
					<td>
						<a href="${ base }music/MusicAlbum/edit?id=${ album.id }&sid=${ album.musicSinger.id }"
						   class="layui-btn layui-btn-xs">编辑</a>
						<a href="${ base }music/MusicAlbum/delete?id=${ album.id }&sid=${ album.musicSinger.id }"
