...
<div class="layui-card">
	<div class="layui-card-header">
        <span class="card-title">音乐专题</span>
		<c:import url="../menubar.jsp?module=music" />
	</div>
	<div class="layui-card-body">
		<form id="searchForm" action="${ base }music/MusicSpecial/list" method="get" class="layui-form">
		...
		<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
			...
			<c:forEach items="${ page.list }" var="special">
				<tr>
					<td>${ special.id }</td>
					<td>${ special.title }</td>
					<td>${ special.type.attrValue }</td>
					<td>
						<fmt:formatDate value="${ special.updateTime }" pattern="yyyy-MM-dd hh:mm" />
					</td>
					<td>${ special.hits }</td>
					<td>
						<a href="${ base }music/MusicSpecial/edit?id=${ special.id }"
						   class="layui-btn layui-btn-xs">编辑</a>
						<a href="${ base }music/MusicSpecial/load?id=${ special.id }"
						   class="layui-btn layui-btn-xs">导入音乐</a>
						<a href="${ base }music/MusicSpecial/delete?id=${ special.id }" name="delete"
