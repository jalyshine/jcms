...
<div class="layui-card">
	<div class="layui-card-header">
		<span class="card-title">投票管理</span>
		<c:import url="../menubar.jsp?module=vote" />
	</div>
	<div class="layui-card-body">
		<form action="${ base }vote/Vote/list" method="get" class="layui-form" id="searchForm">
		...
		<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
			...
			<tbody>
			<c:forEach items="${ page.list }" var="vote">
				<tr>
					<td>${ vote.id }</td>
					<td>${ vote.title }</td>
					<td><fmt:formatDate value="${ vote.fromTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td><fmt:formatDate value="${ vote.toTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td><fmt:formatDate value="${ vote.updateTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td>
						<a href="${ base }vote/Vote/edit?id=${ vote.id }"
						   class="layui-btn layui-btn-xs">编辑</a>
						<a href="${ base }vote/Vote/delete?id=${ vote.id }" name="delete"
