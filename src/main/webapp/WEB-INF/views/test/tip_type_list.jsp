...
<div class="layui-card">
	<div class="layui-card-header">
		<span class="card-title">贴士分类</span>
		<%--<c:import url="../menubar.jsp?module=tip" />--%>
	</div>
	<jfp:SqlDataSource id="DB1" cmd="SELECT * FROM tip_type WHERE site_id = ${ sessionScope.current_site }" />
	<div class="layui-card-body">
		<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
			...
			<tbody>
			<jfp:Repeater dataSourceID="DB1" var="type">
				<tr>
					<td>${ type.id }</td>
					<td>${ type.name }</td>
					<td>${ type.description }</td>
					<td>
						<a href="${ base }tip/TipType/edit?id=${ type.id }"
						   class="layui-btn layui-btn-xs">编辑</a>
						<a href="${ base }tip/TipType/delete?id=${ type.id }" name="delete"
