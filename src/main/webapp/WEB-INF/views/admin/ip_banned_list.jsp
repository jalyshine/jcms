...
<div class="layui-card">
	<div class="layui-card-header">
		<span class="card-title">禁止IP管理</span>
		<c:import url="../menubar.jsp?module=admin" />
	</div>
	<div class="layui-card-body">
		<form action="${ base }admin/IpBanned/list" method="get" class="layui-form" id="searchForm">
		...
		<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
			...
			<tbody>
			<c:forEach items="${ page.list }" var="ipBanned">
				<tr>
					<td>${ ipBanned.id }</td>
					<td>${ ipBanned.ip }</td>
					<td>
						<fmt:formatDate value="${ ipBanned.expires }" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td>
						<a href="${ base }admin/IpBanned/edit?id=${ ipBanned.id }"
						   class="layui-btn layui-btn-xs">编辑</a>
						<a href="${ base }admin/IpBanned/delete?id=${ ipBanned.id }" name="delete"
