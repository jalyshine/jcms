...
<div class="layui-card">
	<div class="layui-card-header">
		<span class="card-title">友情链接</span>
		<c:import url="../menubar.jsp?module=link" />
	</div>
	<div class="layui-card-body">
		<form action="${ base }link/FriendlyLink/list" method="get" class="layui-form" id="searchForm">
		...
		<table lay-filter="items" lay-even class="layui-table" lay-size="sm">
			...
			<c:forEach items="${ page.list }" var="link">
				<tr>
					...
					<td>${ link.name }</td>
					<td>
						<c:if test="${ link.logo == null || link.logo == '' }">-</c:if>
						<c:if test="${ link.logo != null && link.logo != '' }">
							<img src="${ host }/${ link.logo }" style="height: 30px;"/>
						</c:if>
					</td>
					<td>${ link.friendlyLinkType.name }</td>
					<td>${ link.passed == "true"?"通过":"未通过" }</td>
					<td>
						<a href="${ base }link/FriendlyLink/edit?id=${ link.id }"
						   class="layui-btn layui-btn-xs">编辑</a>
						<a href="${ base }link/FriendlyLink/delete?id=${ link.id }" name="delete"
